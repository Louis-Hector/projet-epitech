...
void condition_s_c(const char *format, int *i, int *total_character
    , va_list list)
{
    char *s;
    char c;
...
void condition_d_i(const char *format, int *i, int *total_character
    , va_list list)
{
    int nb;
...
void condition_modulo(const char *format, int *i, int *total_character
    , va_list list)
{
...
int mini_printf(const char *format, ...)
{
    ...
    for (i; format[i] != '\0'; i++) {
        ...
        condition_modulo(format, &i, &total_character, list);
        if (format[i] == 0)
            break;
        if (format[i] != '%') {
            my_putchar(format[i]);
