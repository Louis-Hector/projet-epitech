...
namespace nts {
    ...
    nts::Tristate Gate4081AND::comAnd(std::size_t pin1, std::size_t pin2)
    {
    ...
    nts::Tristate Gate4081AND::compute(std::size_t pin)
    {
        ...
        if (element != output.end())
            _pin_value[pin] = comAnd(element->second.first, element->second.second);
        else
