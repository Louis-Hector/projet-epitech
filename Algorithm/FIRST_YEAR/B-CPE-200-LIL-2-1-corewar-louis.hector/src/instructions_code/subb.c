...
/**
 * @brief Executes the SUB operation for a process.
 *
 * This function performs the subtraction of two register values and
 * stores the result in a third register. It also updates the program
 ...
 */
int affect_value_subb(processes_t *process
    , params_values_t *values, corewar_t *global)
{
    ...
    if (!process->is_ready) {
        ...
        process->wait = op_tab[4].nbr_cycles;
        return 0;
    }
    ...
    process->pc += (1 + calculate_new_pc(*values, 3, 4)) % MEM_SIZE;
    change_carry(sum, process);
    return 0;
}

/**
 * @brief Handles the SUB instruction for a process.
 *
 * This function decodes the SUB instruction from the arena, validates
 * the argument types, and calls the function to execute the operation.
 * The SUB instruction subtracts the values of two registers and stores
...
int make_sub(corewar_t *global, processes_t *process)
{
    ...
    if (values.args[0].param != T_REG
        ...
        return 1;
    return affect_value_subb(process, &values, global);
}
