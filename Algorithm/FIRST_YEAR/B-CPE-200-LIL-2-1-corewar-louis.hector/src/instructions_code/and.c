...
/**
 * @brief Executes the AND operation for a process.
 *
 * This function performs a bitwise AND operation between two values and
 ...
 */
int affect_value_and(processes_t *process
    , params_values_t *values, corewar_t *global)
{
    ...
    if (!process->is_ready) {
        ...
        process->wait = op_tab[5].nbr_cycles;
        return 0;
    }
    process->is_ready = false;
    content = values->args[0].value_direct & values->args[1].value_direct;
    process->registre[values->args[2].value_regi] = content;
    process->pc = 1 + calculate_new_pc(*values, 3, 5) % MEM_SIZE;
    change_carry(content, process);
    return 0;
}

/**
 * @brief Handles the AND instruction for a process.
 *
 * This function decodes the AND instruction from the arena, validates
 * the argument types, and calls the function to execute the operation.
 * The AND instruction performs a bitwise AND between two values and
...
int make_and(corewar_t *global, processes_t *process)
{
    ...
    return affect_value_and(process, &values, global);
}
